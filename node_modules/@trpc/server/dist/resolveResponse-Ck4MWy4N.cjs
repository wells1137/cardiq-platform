...
const jsonContentTypeHandler = {
	...
	async parse(opts) {
		...
		if (isBatchCall && typeof maxBatchSize === "number" && paths.length > maxBatchSize) throw new require_tracked.TRPCError({
			code: "BAD_REQUEST",
		...
		const calls = await Promise.all(paths.map(async (path, index) => {
			const procedure = await require_tracked.getProcedureAtPath(opts.router, path);
			return {
				batchIndex: index,
				path,
				procedure,
...
const formDataContentTypeHandler = {
	...
	async parse(opts) {
		const { req } = opts;
		if (req.method !== "POST") throw new require_tracked.TRPCError({
		...
		const procedure = await require_tracked.getProcedureAtPath(opts.router, opts.path);
		return {
			accept: null,
			calls: [{
				batchIndex: 0,
	...
	async parse(opts) {
		...
		return {
			calls: [{
				...
				getRawInput: getInputs.read,
				result: getInputs.result,
				procedure: await require_tracked.getProcedureAtPath(opts.router, opts.path)
			}],
			isBatchCall: false,
			accept: null,
			type: "mutation",
...
function caughtErrorToData(cause, errorOpts) {
	...
	const untransformedJSON = { error: require_getErrorShape.getErrorShape({
	...
	const transformedJSON = require_tracked.transformTRPCResponse(router._def._config, untransformedJSON);
...
async function resolveResponse(opts) {
		...
		return {
			...
			create: async (info) => {
				...
				try {
					...
					result = [require_tracked.getTRPCErrorFromUnknown(cause), void 0];
				}
			}
		};
	});
	...
	*/
	...
	try {
		...
		if (info.isBatchCall && !allowBatching) throw new require_tracked.TRPCError({
			code: "BAD_REQUEST",
			message: `Batching is not enabled on the server`
		});
		/* istanbul ignore if -- @preserve */
		if (isStreamCall && !info.isBatchCall) throw new require_tracked.TRPCError({
		...
		const rpcCalls = info.calls.map(async (call) => {
			...
			try {
				...
				if (!methodMapper[proc._def.type].includes(req.method)) throw new require_tracked.TRPCError({
					code: "METHOD_NOT_SUPPORTED",
		...
		if (!info.isBatchCall) {
			...
			switch (info.type) {
				...
				case "query": {
					...
					const res = error ? { error: require_getErrorShape.getErrorShape({
						config,
						ctx: ctxManager.valueOrUndefined(),
						error,
						input: call.result(),
					...
					return new Response(JSON.stringify(require_tracked.transformTRPCResponse(config, res)), {
		...
		if (info.accept === "application/jsonl") {
			...
			const stream = jsonlStreamProducer((0, import_objectSpread2.default)((0, import_objectSpread2.default)({}, config.jsonl), {}, {
				...
				data: rpcCalls.map(async (res, index) => {
					...
					*/
					const iterable = require_observable.isObservable(result.data) ? require_observable.observableToAsyncIterable(result.data, opts.req.signal) : Promise.resolve(result.data);
		...
		*/
		...
		const results = (await Promise.all(rpcCalls)).map((res) => {
			...
			if (error) return res;
			if (isDataStream(result.data)) return [new require_tracked.TRPCError({
				code: "UNSUPPORTED_MEDIA_TYPE",
				message: "Cannot use stream-like response in non-streaming request - use httpBatchStreamLink"
			}), void 0];
			return res;
		...
		return new Response(JSON.stringify(require_tracked.transformTRPCResponse(config, resultAsRPCResponse)), {
...
//#endregion
Object.defineProperty(exports, 'Unpromise', {
  enumerable: true,
  get: function () {
    return Unpromise;
  }
...
Object.defineProperty(exports, 'getRequestInfo', {
  enumerable: true,
  get: function () {
...
Object.defineProperty(exports, 'iteratorResource', {
  enumerable: true,
  get: function () {
    return iteratorResource;
  }
});
Object.defineProperty(exports, 'jsonlStreamConsumer', {
  enumerable: true,
  get: function () {
    return jsonlStreamConsumer;
  }
});
Object.defineProperty(exports, 'jsonlStreamProducer', {
  enumerable: true,
  get: function () {
    return jsonlStreamProducer;
  }
});
Object.defineProperty(exports, 'makeAsyncResource', {
  enumerable: true,
  get: function () {
    return makeAsyncResource;
  }
});
Object.defineProperty(exports, 'makeResource', {
  enumerable: true,
  get: function () {
    return makeResource;
  }
});
Object.defineProperty(exports, 'parseConnectionParamsFromString', {
  enumerable: true,
  get: function () {
    return parseConnectionParamsFromString;
  }
});
Object.defineProperty(exports, 'parseConnectionParamsFromUnknown', {
  enumerable: true,
  get: function () {
    return parseConnectionParamsFromUnknown;
  }
});
...
Object.defineProperty(exports, 'sseHeaders', {
  enumerable: true,
  get: function () {
    return sseHeaders;
  }
});
Object.defineProperty(exports, 'sseStreamConsumer', {
  enumerable: true,
  get: function () {
    return sseStreamConsumer;
  }
});
Object.defineProperty(exports, 'sseStreamProducer', {
  enumerable: true,
  get: function () {
    return sseStreamProducer;
  }
});
Object.defineProperty(exports, 'takeWithGrace', {
  enumerable: true,
  get: function () {
    return takeWithGrace;
  }
});
Object.defineProperty(exports, 'throwAbortError', {
  enumerable: true,
  get: function () {
